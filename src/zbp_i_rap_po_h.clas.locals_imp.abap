...
CLASS lhc_header IMPLEMENTATION.
  ...
  METHOD setstatusnew.
    READ ENTITIES OF zi_rap_po_h IN LOCAL MODE
      ENTITY Header
        FIELDS ( Status )
        WITH CORRESPONDING #( keys )
      RESULT DATA(lt_headers).

    DATA lt_update TYPE TABLE FOR UPDATE zi_rap_po_h.
    ...
    IF lt_update IS NOT INITIAL.
      MODIFY ENTITIES OF zi_rap_po_h IN LOCAL MODE
        ENTITY Header
...
CLASS lhc_item IMPLEMENTATION.
  ...
  METHOD earlynumbering_cba_items.
    ...
    LOOP AT entities INTO DATA(ls_header_entity).
      ...
      "Active table: po_id/item_id
      SELECT MAX( item_id )
        FROM zrap_po_i
        WHERE po_id = @lv_po_id
        INTO @lv_max_active_numc.

      "Draft table: poid/itemid (based on your table definition)
      SELECT MAX( itemid )
        FROM zrap_d_po_i
        WHERE poid = @lv_po_id
  ...
  METHOD defaultcurrency.
    ...
    DATA lt_update TYPE TABLE FOR UPDATE zi_rap_po_i.
